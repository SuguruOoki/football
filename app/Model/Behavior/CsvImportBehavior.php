<?php
    ...
    class CsvImportBehavior extends ModelBehavior
    {
        ...
        public function importCSV(
            Model &$Model, $file, $fixed = array(), $returnSaved = false
        )
        {
            ...
            while (($row = $this->_getCSVLine($Model, $handle)) !== false) {
                ...
                foreach ($header as $k => $col) {
                    ...
                    if (strpos($col, '.') !== false) {
                        ...
                        if (isset($keys[2])) {
                            ...
                            $data[$keys[0]][$keys[1]] = (isset($row[$k]))
                                ? $row[$k] : '';
                        }
                    } else {
                        $data[$Model->alias][$col] = (isset($row[$k]))
                            ? $row[$k] : '';
                    }
                }
                $data = Set::merge($data, $fixed);
                $Model->create();
                $Model->id = isset($data[$Model->alias][$Model->primaryKey])
                    ? $data[$Model->alias][$Model->primaryKey] : false;
                ...
                $Model->set($data);
                if (!$Model->validates()) {
                    $this->errors[$Model->alias][$i]['validation']
                        = $Model->validationErrors;
                    $error = true;
                    $this->_notify(
                        $Model, 'onImportError',
                        $this->errors[$Model->alias][$i]
                    );
                }
                // save the row
                if (!$error
                    && !$Model->saveAll(
                        $data, array('validate' => false, 'atomic' => false)
                    )
                ) {
                    $this->errors[$Model->alias][$i]['save'] = sprintf(
                        __d('utils', '%s for Row %d failed to save.'),
                        $Model->alias, $i
                    );
                    $error = true;
                    $this->_notify(
